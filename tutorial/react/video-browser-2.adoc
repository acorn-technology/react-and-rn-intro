...
----
+
*Line 6:* When user enters text the `onChange` handler is called, the state searchTerm field is updated with new user input, and finally React is triggered to rerender. 
+
*Line 5:* When rendering is complete, the `value` is set to `this.state.searchTerm`. 
+
